/**
 * TaskTableModel.java -----------------------------------------------------------------------------
 * Project           Memoranda Package           net.sf.memoranda.ui Original author   Alex V.
 * Alishevskikh [alexeya@gmail.com] Created           18.05.2005 15:16:11 Revision info
 * $RCSfile: TaskTableModel.java,v $ $Revision: 1.7 $ $State: Exp $
 * <p>
 * Last modified on  $Date: 2005/12/01 08:12:26 $ by  $Author: alexeya $
 *
 * @VERSION@
 * @COPYRIGHT@
 * @LICENSE@
...
public class TaskTableModel extends AbstractTreeTableModel implements TreeTableModel {
  ...
  public Object getChild(Object parent, int index) {
    if (parent instanceof Project) {
      if (activeOnly()) {
        return CurrentProject.getTaskList().getActiveSubTasks(null, CurrentDate.get())
            .toArray()[index];
      } else {
    ...
    Task t = (Task) parent;
    if (activeOnly()) {
      return CurrentProject.getTaskList().getActiveSubTasks(t.getID(), CurrentDate.get())
          .toArray()[index];
    } else {
