/**
 * ProjectPackager.java Created on 16.03.2003, 18:41:29 Alex Package: net.sf.memoranda.util
 *
...
public class ProjectPackager {
  ...
  public static void unpack(File zipfile) {
    try {
      ...
      BufferedReader in = new BufferedReader(
          new InputStreamReader(zip.getInputStream(info), "UTF-8"));
      ...
      for (files = zip.entries(); files.hasMoreElements(); ) {
        ...
        if ((!ze.getName().equals("__PROJECT_INFO__")) && (!ze.isDirectory())) {
          FileOutputStream out = new FileOutputStream(
              JN_DOCPATH + prj.getID() + "/" + ze.getName());
  ...
  public static void packDirectory(String startingDirectory,
    ...
    System.out.println(
        "Path=" + stDirectory.getPath() + ";length=" + stDirectory.getPath().length() + "==>"
            + theFiles[0]);
