/**
 * AppList.java Created on 24.03.2003, 14:21:50 Alex Package: net.sf.memoranda.util
 *
  ...
  public String getCommandLine(String appId, String fPath) {
    ...
    for (int i = 0; i < apps.size(); i++) {
      if (apps.get(i).getAttribute("id").getValue().equals(appId)) {
        ...
        return app.getAttribute("findPath").getValue() + "/" + app.getAttribute("exec").getValue()
            + " " + pt;
      }
    }
    return null;
  ...
  public String[] getCommand(String appId, String fPath) {
    ...
    for (int i = 0; i < apps.size(); i++) {
      if (apps.get(i).getAttribute("id").getValue().equals(appId)) {
        ...
        String exec =
            app.getAttribute("findPath").getValue() + "/" + app.getAttribute("exec").getValue();
